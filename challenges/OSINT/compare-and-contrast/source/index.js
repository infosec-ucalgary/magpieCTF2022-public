...
const PORT = 8000;
const ROOT = process.env.NODE_APP_ROOT || "./public";
const LOG_FILE_PATH = ".log";

const INTERCEPT_FILE = "data/intercept.txt";
...
function checkCiphertext(reqBody, response){
  let ciphertext = formatString("lh gv iu dv jd kv qb rm qn ro uc rw");

  try{
    let reqData = JSON.parse(reqBody);
    let matching = ciphertext === formatString(reqData.answer);
    ...
    send403(response, err);
    return;
  }
}

function checkAnswer(reqBody, response){
  let desiredFile = `${ROOT}/${INTERCEPT_FILE}`;
  let reqData, resData;

  fs.readFile(desiredFile, (err, data) => {
    if(err){
      send500(response, err, "This should not be happening, please contact a challenge admin asap");
      return;
    }

    try{
      reqData = JSON.parse(reqBody);

      let matching = formatString("omniflagsstoleallcontrol") === formatString(reqData.answer);
...
function onRequest(request, response){
  ...
  request.on("end", () => {
    ...
    if(request.method === "POST"){
      ...
      switch(reqPath){
        case("/check-cipher"):
          checkCiphertext(reqBody, response);
          break;
        case("/check-plain"):
          checkAnswer(reqBody, response);
          break;
        default:
          send404(response);
      }
